...
chi1= loadtxt('refs/o-pulse-1e2-w20.YPP-eps_along_E')
chi2= loadtxt('refs/o-pulse-1e2-w60.YPP-eps_along_E')
...
plot(chi1[:,0],chi1[:,1],'-',label='$t=20$ s',lw=3)
plot(chi2[:,0],chi2[:,1],'-',label='$t=60$ s')
legend(loc=1)
twinx()               
plot(pul1[:,0],pul1[:,1]**2+pul1[:,4]**2,'--',label='$t=20$ s')
plot(pul1[:,0],pul2[:,1]**2+pul2[:,4]**2,'--',label='$t=60$ s')
legend(loc=2)
